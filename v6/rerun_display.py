...
"""Re-run the display chain from a previous run's saved composite, into a fresh output dir.
    ...
    python v6/rerun_display.py --in-dir /home/slavik/tmp/eclipse_v6_rere \
...
The cut is `v6-stage3_composite.npy` — the output of `crop_and_save_composite`, physical brightness
with `NO_DATA` still in the moon. Everything before it (ingest, moon detection, registration,
calibration, re-registration, the merge) is skipped, along with its need for frames, `v6-calib.pkl`
...
def _parse_args():
    ...
    ap.add_argument("--in-dir", type=Path, required=True,
                    help="previous run's output dir, holding v6-stage3_composite.npy (read-only)")
...
def load_composite_into_ctx(ctx: S3.Stage3Context, in_dir: Path, seed_moon=None) -> None:
    ...
    """
    path = in_dir / "v6-stage3_composite.npy"
    ...
    var_path = in_dir / "v6-stage3_variance.npy"
    if var_path.exists():
...
if __name__ == "__main__":
    ...
    if args.copy_composite:
        for name in ("v6-stage3_composite.npy", "v6-stage3_variance.npy"):
    ...
    with open(out_dir / "v6-rerun-display.json", "w") as fh:
        json.dump(meta, fh, indent=2)
    print(f"\nDone. Outputs in {out_dir} (provenance in v6-rerun-display.json)")
