...
v6 is currently a self-contained copy of v5 (no cross-version imports; the `vN/eclipse_vN/`
convention v3 established), with all internal naming renamed to match. No logic has changed
yet. The rest of this README describes v5's history relative to v2, which still applies verbatim
to v6.
...
| `eclipse_v6/merge.py` | per-frame brightness, inverse-variance merge, common moon blanking, `NO_DATA` sentinel; also keeps every exposure's own weight map (before summing) and writes it as `v6-stage3_weights.npy` — large by design, see below |
...
| `eclipse_v6/inputs.py` | adds `set_calibration` / `effective_exposure` / `load_radiance` to the source layer; `NefSource` also finds `lights/`/`darks/` under the folder it's given, and `load_gray` dark-subtracts (via `set_dark_model`) so registration, the calibration, and the merge all see the same cleaned values |
| `eclipse_v6/darkcal.py` | per-pixel dark-current model `bias + rate * t`, fit by OLS over every `darks/` frame at once against the plain EXIF-reported `t` — deliberately no shutter-time correction (see its module docstring for why matched-by-label subtraction doesn't need one); writes `v6-dark.pkl` |
...
| `pipeline.py` | the runner: **dark model** → stage0 → 1 → 2 → calibration → re-registration → recalibration → 3 |
...
python v6/test_darkcal_synthetic.py               # verify the dark-model numerics (no GPU)
...
python v6/pipeline.py --input-mode nef --dark-model off   # skip the dark-current subtraction
...
`v6-stage3_weights.npy` — shape `(n_exp, H_crop, W_crop)` float32, one full-resolution
weight map per exposure in the merge's cross-exposure chain, cropped identically to
`v6-stage3_composite.npy`. `v6-stage3_weights.npy[k]` is exactly the weight exposure
`v6-stage3_weights_exposures.npy[k]` (that array's reported shutter time, seconds)
contributed to `composite_variance` at every pixel, *before* summing across exposures —
`v6-stage3_weights.npy.sum(axis=0) == 1 / v6-stage3_variance.npy` at every covered pixel.
...
`v6-stage3_weights_exposures.npy` gives the dominant exposure per pixel cheaply, if that is
all you need.
