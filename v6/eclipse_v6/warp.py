...
Originally lifted out of v2's merge because multiple callers needed it: the radiometric
merge (`merge.py`) and the response-curve calibration (`calib.py`) — v6's own legacy-merge
copy (`stage3.warp_merge_to_composite`) has since been removed. Keeping one copy matters
more than it looks — the chain composition order and the sign convention of `rotation_deg`
are the only thing tying an exposure's pixels to the reference exposure's pixels, and two
divergent copies would silently disagree.
"""
