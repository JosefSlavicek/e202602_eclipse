"""Read cached v2 pickles under the v6 package name.

A v2 run's `v2-stage0/1/2.pkl` name their classes as `eclipse_v2.stage0.ImageInfo`, so
`pickle.load` re-imports `eclipse_v2`.  v6 is a vendored copy — the classes are identical
...
def install_v2_pickle_aliases() -> list[str]:
    """Alias `eclipse_v2[.sub]` -> `eclipse_v6[.sub]` in `sys.modules`. Returns names installed.
