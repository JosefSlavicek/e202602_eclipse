...
def optimize_poses_and_debug(
    ...
    for exposure_time in sorted(exposure_groups.keys()):
        ...
        Image.fromarray(crop_single).save(debug_img_dir / f"v5-stage1_debugimg_{exposure_time:.6f}_random.png")
        Image.fromarray(crop_avg).save(debug_img_dir / f"v5-stage1_debugimg_{exposure_time:.6f}_average.png")
        frames = []
        for w in warped_list:
            c = (w[i_lo:i_hi, j_lo:j_hi].cpu().numpy() * 255).clip(0, 255).astype(np.uint8)
            frames.append(Image.fromarray(c))
        frames[0].save(
            debug_img_dir / f"v5-stage1_debugimg_{exposure_time:.6f}_anim.gif",
            save_all=True,
