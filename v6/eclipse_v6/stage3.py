...
from eclipse_v6.utils import load_grayscale, apply_transform_single, compute_weighted_average, moon_median
...
def crop_and_save_composite(ctx: Stage3Context) -> None:
    ...
    if ctx.exposure_weights_crop is not None:
        ...
        np.save(out_dir / "v6-stage3_weights.npy", ctx.exposure_weights_crop)
        np.save(out_dir / "v6-stage3_weights_exposures.npy", ctx.exposure_weight_times)
        saved.append(out_dir / "v6-stage3_weights.npy")
        saved.append(out_dir / "v6-stage3_weights_exposures.npy")
...
def run(workdir: Path, source=None, use_legacy_merge: bool = False) -> None:
    ...
    `v6-calib.pkl` alongside the stage pickles. `use_legacy_merge=True` reproduces v2.
    """
