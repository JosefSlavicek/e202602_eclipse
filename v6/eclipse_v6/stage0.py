...
from eclipse_v6.coords import cartesian_to_polar, polar_to_cartesian, circumcenter
from eclipse_v6.utils import (
    load_grayscale,
    compose_transforms,
    remove_lowfeq,
    fill_bottom,
...
def prune_moon_info_for_radius_outliers(exposure_groups: dict) -> None:
    ...
    Seeds the reference from the first internally-consistent group in
    exposure-sorted order, not group index 0 -- the shortest/longest
    exposures in a bracket are typically the LEAST reliable moon detections
    (bright partial phase / saturated corona), so trusting keys[0] blindly
    can leave prev_avg_radius unseeded and crash on the very first group.
    """
    def radius(ii):
        return ii.moon[2]
    ...
    for k in keys:
        radii = [radius(ii) for ii in exposure_groups[k]]
...
def interpolate_missing_moons(image_infos: list, exposure_groups: dict) -> tuple:
    ...
    keys = sorted(exposure_groups.keys())
    # Seed from the first group (in exposure order) that has any direct
    # detection left after pruning -- leading groups can be entirely
    # detection-free (see prune_moon_info_for_radius_outliers), so trusting
    # keys[0] to have a direct detection isn't safe.
    last_avg_radius = None
    for k in keys:
        with_moon_seed = [ii for ii in exposure_groups[k] if ii.moon is not None]
        if with_moon_seed:
            last_avg_radius = float(np.mean([ii.moon[2] for ii in with_moon_seed]))
            break
