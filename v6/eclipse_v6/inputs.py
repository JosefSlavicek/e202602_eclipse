"""Input sources for the v5 pipeline: JPG, atmosphere-distorted JPG, real NEF, and
NEF+injected-corona.
...
class FrameSource(ABC):
    ...
    _atmosphere = None     # eclipse_v6.atmosphere.Trend, optional -- see set_atmosphere_correction
    ...
    # --- v6 static-atmosphere correction (plan0.md round 3) ---------------------------
    def set_atmosphere_correction(self, trend) -> None:
        """`trend`: an eclipse_v6.atmosphere.Trend, fitted from this run's own calibration
        frames. Every subsequent load_gray/load_rgb call is de-warped through it."""
        self._atmosphere = trend

    def _dewarp(self, arr: torch.Tensor, ii) -> torch.Tensor:
        """No-op unless set_atmosphere_correction() was called. CPU round-trip (scipy
        map_coordinates isn't GPU-native) -- fine at once-per-frame-load cost, not a hot
        loop, but worth knowing if this ever needs to move onto the GPU path."""
        if self._atmosphere is None:
            return arr
        from eclipse_v6 import atmosphere as A
        out = A.dewarp(arr.detach().cpu().numpy(), float(ii.timestamp), self._atmosphere)
        return torch.from_numpy(out).to(device=arr.device, dtype=arr.dtype)
