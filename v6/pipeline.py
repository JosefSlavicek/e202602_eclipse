...
from eclipse_v6 import darkcal as dc
from eclipse_v6 import reregister as rr
from eclipse_v6 import stage0 as s0
from eclipse_v6 import stage1 as s1
from eclipse_v6 import stage2 as s2
from eclipse_v6.inputs import make_source, attach_source
...
def _parse_args():
    ap = argparse.ArgumentParser(description="Eclipse v6 pipeline")
    ap.add_argument("--input-mode", choices=["jpg", "atmosphere", "nef", "inject"],
                    default=os.environ.get("ECLIPSE_V6_INPUT_MODE", "jpg"))
    ...
    ap.add_argument("--atmosphere-dir", type=Path,
                    default=Path(os.environ.get("ECLIPSE_V6_ATMOSPHERE_DIR",
    ...
    ap.add_argument("--nef-dir", type=Path,
                    default=Path(os.environ.get("ECLIPSE_V6_NEF_DIR", "/home/slavik/e202602_eclipse/my_raws")))
    ap.add_argument("--dark-dir", type=Path, default=None,
                    help="Folder of dark-frame .NEF at the same exposure times as the "
                         "lights. Default <nef-dir>/darks (mirrors --nef-dir's own "
                         "<nef-dir>/lights). Only used in --input-mode nef/inject with "
                         "--dark-model on.")
    ap.add_argument("--dark-model", choices=["on", "off"], default="on",
                    help="Fit a per-pixel dark-current model (bias + rate x reported "
                         "exposure time) from --dark-dir and subtract it from every frame "
                         "before stage 0, so registration, the light calibration and the "
                         "final merge all see dark-subtracted values. Ignored outside "
                         "--input-mode nef/inject.")
    ap.add_argument("--workdir", type=Path, default=None,
                    help="Default /home/slavik/tmp/eclipse_v6_run, or $ECLIPSE_V6_WORKDIR. "
    ...
    ap.add_argument("--exposure-group-subsample", type=int,
                    default=int(os.environ.get("ECLIPSE_V6_EXPOSURE_GROUP_SUBSAMPLE", "1")),
...
if __name__ == "__main__":
    ...
    elif "ECLIPSE_V6_WORKDIR" in os.environ:
        WORKDIR = Path(os.environ["ECLIPSE_V6_WORKDIR"])
    else:
        WORKDIR = Path("/home/slavik/tmp/eclipse_v6_run"
                       + ("_atmosphere" if args.input_mode == "atmosphere" else ""))
    WORKDIR.mkdir(parents=True, exist_ok=True)

    PK_STAGE0 = WORKDIR / "v6-stage0.pkl"
    PK_STAGE1 = WORKDIR / "v6-stage1.pkl"
    PK_STAGE2 = WORKDIR / "v6-stage2.pkl"
    PK_STAGE2R = WORKDIR / "v6-stage2r.pkl"   # cross_reg redone on calibrated radiance
    PK_CALIB = WORKDIR / "v6-calib.pkl"
    PK_DARK = WORKDIR / "v6-dark.pkl"
    ...
    # --- Dark model ---
    #
    # Fit before anything else touches a frame: load_gray (which registration, the light
    # calibration's sample-gathering, and the final merge all read through) subtracts it
    # once attached, so this has to be in place before stage 0, not bolted on at the end.
    ...
    if args.dark_model == "on" and source.kind in ("nef", "inject"):
        print("\n=== Dark model: per-pixel bias + rate from the darks/ bracket ===")
        dark_dir = args.dark_dir if args.dark_dir is not None else args.nef_dir / "darks"
        dark_model = dc.run(dark_dir, out_pkl=PK_DARK)
        source.set_dark_model(dark_model)
    ...
    if source.kind == "atmosphere":
        ...
        with open(WORKDIR / "v6-input-atmosphere.json", "w") as fh:
