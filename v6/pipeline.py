...
from eclipse_v6 import atmosphere as atmo
...
def _parse_args():
    ...
    ap.add_argument("--workdir", type=Path, default=None,
                    help="Default /home/slavik/tmp/eclipse_v6_run, or $ECLIPSE_V5_WORKDIR. "
    ...
    ap.add_argument("--stop-after-stage", type=int, default=None, choices=[0, 1, 2, 3],
                    help="Exit right after this stage finishes instead of continuing to "
                         "stage 3's merge. v6/plan0.md round 4 only needs stages 0-1 "
                         "(registration), not the ~50 min merge.")
    ap.add_argument("--no-atmosphere-correction", action="store_true",
                    help="v6/plan0.md: by default every run fits and applies the static-"
                         "atmosphere correction from this run's OWN calibration-capable "
                         "frames (no separate precompute step). Pass this to get the "
                         "uncorrected baseline instead.")
...
if __name__ == "__main__":
    ...
    # --- v6 static-atmosphere correction (plan0.md) ---
    # On by default, for every input mode: fits the correction from this run's OWN
    # calibration-capable frames (see eclipse_v6.atmosphere.calibrate_from_source), no
    # separate build_atmosphere_correction.py step. Cheap relative to the rest of the
    # pipeline (dogsnr + a handful of plate solves, CPU, well under a minute) and has to
    # run before ANY frame is loaded, so it happens unconditionally here regardless of
    # --start-stage -- stage 2/3 reload full-res pixels too, and they must see the same
    # correction stage 0/1 registered under.
    if args.no_atmosphere_correction:
        print("Atmosphere correction: OFF (--no-atmosphere-correction)")
    else:
        print("Atmosphere correction: fitting from this run's calibration-capable frames...")
        calib_infos = s0.get_image_infos(source)
        trend = atmo.calibrate_from_source(calib_infos, device)
        source.set_atmosphere_correction(trend)
        pkl_path = WORKDIR / "v5-atmosphere-trend.pkl"
        with open(pkl_path, "wb") as fh:
            pickle.dump(trend, fh)
        print(f"Atmosphere correction: fitted (t_mean={trend.t_mean:.2f}), archived to {pkl_path}")
    ...
    if args.stop_after_stage is not None and args.stop_after_stage <= 1:
        print(f"\nStopping after stage {args.stop_after_stage} (--stop-after-stage).")
        sys.exit(0)
