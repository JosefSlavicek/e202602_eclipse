...
only in the images handed to it:
  ...
  control  stored values, scaled by the pickled `gamma` — reproduces stage 2 exactly.
           Its delta against `v6-stage2.pkl` must be ~0; that is what makes the other two
...
control has been shown to reproduce.
    ...
    python v6/compare_registration_radiance.py --workdir /home/slavik/tmp/eclipse_v6

Reads `v6-stage1.pkl`, `v6-stage2.pkl`, `v6-calib.pkl` from the workdir. Writes nothing
except the JSON report (`--out`, default `<workdir>/v6-regcheck.json`).
"""
...
def _parse_args():
    ...
    ap.add_argument("--workdir", type=Path, default=Path("/home/slavik/tmp/eclipse_v6"),
                    help="holds v6-stage1.pkl, v6-stage2.pkl, v6-calib.pkl")
    ...
    ap.add_argument("--out", type=Path, default=None,
                    help="JSON report path; default <workdir>/v6-regcheck.json")
...
def main():
    ...
    out_path = args.out or (workdir / "v6-regcheck.json")
    ...
    with open(workdir / "v6-stage1.pkl", "rb") as fd:
        exposure_groups = pickle.load(fd)
        _reg = pickle.load(fd)
        opt_results = pickle.load(fd)
    with open(workdir / "v6-stage2.pkl", "rb") as fd:
        cross_reg = pickle.load(fd)
        gamma_by_pair = pickle.load(fd)
    calib = ca.load(workdir / "v6-calib.pkl")
