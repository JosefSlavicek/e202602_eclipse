...
from eclipse_v1.utils import load_grayscale
...
def discrepancy_batched_fourier3(
    ...
    for img, moon_center, moon_radius in list_of_all:
        ...
        polar_img, _ = cartesian_to_polar(
            img, moon_center, moon_radius, radius_max, n_r, n_theta
        )
        ...
        img = polar_to_cartesian(
            polar_img, moon_center, moon_radius, radius_max, H_img, W_img
        )
        if blur_sigma > 0:
            img = gaussian_blur(img, blur_sigma)
        mask = torch.ones_like(polar_img)
        mask = polar_to_cartesian(
            mask, moon_center, moon_radius, radius_max, H_img, W_img
        )
