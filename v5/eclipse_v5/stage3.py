...
import eclipse_v5.stage0  # noqa: F401
from eclipse_v5.stage0 import find_moon
from eclipse_v5.coords import cartesian_to_polar, polar_to_cartesian
from eclipse_v5.merge import NO_DATA
from eclipse_v5.utils import load_grayscale, apply_transform_single, compute_weighted_average, moon_median
from eclipse_v5.warp import (
    build_chains,
...
def crop_and_save_composite(ctx: Stage3Context) -> None:
    ...
    np.save(out_dir / "v5-stage3_composite.npy", radiance_crop)
    saved = [out_dir / "v5-stage3_composite.npy"]
    if ctx.variance_crop is not None:
        np.save(out_dir / "v5-stage3_variance.npy", ctx.variance_crop)
        saved.append(out_dir / "v5-stage3_variance.npy")
    ...
    Image.fromarray((preview * 255).clip(0, 255).astype(np.uint8)).save(
        out_dir / "v5-stage3_composite_preview.png"
    )
    print("Saved " + ", ".join(str(p) for p in saved)
          + f", {out_dir / 'v5-stage3_composite_preview.png'}")
...
def radial_normalize_display(ctx: Stage3Context) -> None:
    ...
    Image.fromarray((ctx.display * 255).clip(0, 255).astype(np.uint8)).save(
        ctx.workdir / "v5-stage3_radial_normalize.png"
    )
    print(f"Saved {ctx.workdir / 'v5-stage3_radial_normalize.png'}")
...
def fft_unsharp_and_save(ctx: Stage3Context) -> None:
    ...
    out_png = ctx.workdir / "v5-stage3_radial_normalize_sharpen_fft_smoothed_diff.png"
...
def rgb_vignette_and_radial_pickle(ctx: Stage3Context) -> None:
    """RGB + vignette PNG and v5-stage3_radial.pkl sidecar."""
    ...
    out_png = ctx.workdir / "v5-stage3_rgb_rescaled.png"
...
def load_calibration(ctx: Stage3Context, source=None, path: Path = None):
    ...
    path = (ctx.workdir / "v5-calib.pkl") if path is None else Path(path)
...
def run(workdir: Path, source=None, use_legacy_merge: bool = False) -> None:
    """Run full stage 3 pipeline; writes v5-stage3_* artifacts under workdir.

    Needs `source` for the radiometric merge (it reads frames through `load_radiance`) and a
    `v5-calib.pkl` alongside the stage pickles. `use_legacy_merge=True` reproduces v2.
    """
