...
Stages 0-2 of v5 are byte-identical to v2's, so a finished v2 workdir is a perfectly good
input: this reads its `v2-stage1.pkl` / `v2-stage2.pkl` through the module-alias shim,
re-saves them under the v5 names, then runs the calibration and stage 3.

    python v5/run_from_v2_cache.py \\
...
def _parse_args():
    ...
    ap.add_argument("--workdir", type=Path,
                    default=Path("/home/slavik/tmp/eclipse_v5_from_v2"),
                    help="where v5 writes; never the cache dir")
...
def import_v2_pickles(cache_dir: Path, workdir: Path) -> None:
    """Re-save the cached v2 stage pickles under the v5 names, via the alias shim."""
    ...
    with open(workdir / "v5-stage1.pkl", "wb") as fd:
        pickle.dump(exposure_groups, fd)
        pickle.dump(reg, fd)
        pickle.dump(opt_results, fd)
    shutil.copyfile(src2, workdir / "v5-stage2.pkl")
...
def main() -> int:
    ...
    from eclipse_v5.stage3 import (
        Stage3Context, build_per_exposure_averages, crop_and_save_composite,
        fft_unsharp_and_save, load_calibration, load_inputs, radial_normalize_display,
        rgb_vignette_and_radial_pickle, warp_merge_to_composite,
    )
    ...
    pk_calib = args.workdir / "v5-calib.pkl"
    ...
    if args.merge != "legacy":
        ...
        exposure_groups, _reg, opt_results = s2.load(args.workdir / "v5-stage1.pkl")
        attach_source(exposure_groups, source)
        with open(args.workdir / "v5-stage2.pkl", "rb") as fd:
