...
def unlock_and_burst():
    ...
    try:
        for main_idx in range(3):
            ...
            do_burst('0.0015s',   '1 EV', '9 images (normal, 4 unders and 4 overs)', 'no_repeat' if main_idx == 2 else 'double_in_single_run', main_idx==0)
            do_burst('0.0769s',   '1 EV', '7 images (normal, 3 unders and 3 overs)', 'no_repeat' if main_idx == 2 else 'double_in_two_runs', False)
            do_burst('0.0012s',   '1 EV', '9 images (normal, 4 unders and 4 overs)', 'no_repeat' if main_idx == 2 else 'double_in_single_run', False)
