...
| `eclipse_v7/flatcal.py` | per-pixel flat field, **high-frequency only and brightness-dependent**: `flats/` is grouped by exposure time (dropping frames with too many saturated pixels and whole groups with too few survivors), each group gets the usual high-pass treatment (sigma=24px Gaussian blur, mirror-padded, subtracted from each dark-subtracted frame first), then every pixel gets its own least-squares line `a(x,y) + b(x,y)*value` fit across groups (falling back to a single brightness-independent map if fewer than 2 groups qualify); `FlatModel.evaluate(value)` clamps the result to `[0.95, 1.05]` at apply time; writes `v7-flat.pkl` |
...
| `test_flatcal_synthetic.py` | plants a radial vignetting pattern + a 3x3 dust shadow across several synthetic exposure groups (plus a too-thin group and an overburned frame to exercise both exclusion filters), asserts the fitted per-pixel line matches an independently-computed ground truth (`np.polyfit`, not the production closed-form solve), that the overburned frame and thin group are correctly excluded and don't change the fit, and that a single-group dataset falls back to `b=0`; separately checks the high-pass step's mirror-padded edges don't bias a uniform frame. No GPU, no files, ~1 s |
