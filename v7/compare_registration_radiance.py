...
only in the images handed to it:
  ...
  control  stored values, scaled by the pickled `gamma` — reproduces stage 2 exactly.
           Its delta against `v7-stage2.pkl` must be ~0; that is what makes the other two
...
control has been shown to reproduce.
    ...
    python v7/compare_registration_radiance.py --workdir /home/slavik/tmp/eclipse_v7

Reads `v7-stage1.pkl`, `v7-stage2.pkl`, `v7-calib.pkl` from the workdir. Writes nothing
except the JSON report (`--out`, default `<workdir>/v7-regcheck.json`).
"""
...
from eclipse_v7 import stage2 as s2                                       # noqa: E402
from eclipse_v7.device import configure_cuda_visible_devices, require_cuda  # noqa: E402
from eclipse_v7.inputs import attach_source, make_source                  # noqa: E402
from eclipse_v7.reregister import (                                       # noqa: E402
...
from eclipse_v7.utils import compute_weighted_average, moon_median        # noqa: E402

# --- constants ------------------------------------------------------------------------
# The radiance path itself lives in `eclipse_v7.reregister`, which is what the pipeline runs;
...
def _parse_args():
    ...
    ap.add_argument("--workdir", type=Path, default=Path("/home/slavik/tmp/eclipse_v7"),
                    help="holds v7-stage1.pkl, v7-stage2.pkl, v7-calib.pkl, and the "
                         "rawprep/corrected caches from the run being checked")
    ap.add_argument("--nef-dir", type=Path, default=Path("/home/slavik/e202602_eclipse/my_raws"))
    ...
    ap.add_argument("--out", type=Path, default=None,
                    help="JSON report path; default <workdir>/v7-regcheck.json")
...
def main():
    ...
    out_path = args.out or (workdir / "v7-regcheck.json")

    source = make_source(nef_dir=args.nef_dir)
    source.set_cache_dirs(raw_cache_dir=workdir / "rawprep", corrected_dir=workdir / "corrected")
    device = torch.device("cuda")

    with open(workdir / "v7-stage1.pkl", "rb") as fd:
        exposure_groups = pickle.load(fd)
        _reg = pickle.load(fd)
        opt_results = pickle.load(fd)
    with open(workdir / "v7-stage2.pkl", "rb") as fd:
        cross_reg = pickle.load(fd)
        gamma_by_pair = pickle.load(fd)
    calib = ca.load(workdir / "v7-calib.pkl")
    ...
    with open(out_path, "w") as fd:
        json.dump({"pairs": rows, "summary": summary,
                   "config": {"space": args.space, "control": want_control,
                              ...
                              "workdir": str(workdir), "input_mode": source.kind}},
                  fd, indent=2)
    print(f"\nWrote {out_path}")
