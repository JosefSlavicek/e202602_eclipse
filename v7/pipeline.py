...
from eclipse_v7 import darkcal as dc
from eclipse_v7 import flatcal as fc
from eclipse_v7 import rawprep as rp
from eclipse_v7 import reregister as rr
from eclipse_v7 import stage0 as s0
from eclipse_v7 import stage1 as s1
from eclipse_v7 import stage2 as s2
from eclipse_v7.inputs import make_source, attach_source
...
def _parse_args():
    ap = argparse.ArgumentParser(description="Eclipse v7 pipeline")
    ap.add_argument("--nef-dir", type=Path,
                    default=Path(os.environ.get("ECLIPSE_V7_NEF_DIR", "/home/slavik/e202602_eclipse/my_raws")))
    ...
    ap.add_argument("--exposure-group-subsample", type=int,
                    default=int(os.environ.get("ECLIPSE_V7_EXPOSURE_GROUP_SUBSAMPLE", "1")),
...
if __name__ == "__main__":
    ...
    elif "ECLIPSE_V7_WORKDIR" in os.environ:
        WORKDIR = Path(os.environ["ECLIPSE_V7_WORKDIR"])
    else:
        WORKDIR = Path("/home/slavik/tmp/eclipse_v7_run")
    WORKDIR.mkdir(parents=True, exist_ok=True)

    PK_STAGE0 = WORKDIR / "v7-stage0.pkl"
    PK_STAGE1 = WORKDIR / "v7-stage1.pkl"
    PK_STAGE2 = WORKDIR / "v7-stage2.pkl"
    PK_STAGE2R = WORKDIR / "v7-stage2r.pkl"   # cross_reg redone on calibrated radiance
    PK_CALIB = WORKDIR / "v7-calib.pkl"
    PK_DARK = WORKDIR / "v7-dark.pkl"
    PK_FLAT = WORKDIR / "v7-flat.pkl"
    ...
    # every light frame before they are averaged together.
    flat_dir = args.flat_dir if args.flat_dir is not None else args.nef_dir / "flats"
