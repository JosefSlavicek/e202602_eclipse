...
import eclipse_v7.stage0  # noqa: F401
from eclipse_v7.stage0 import find_moon
from eclipse_v7.coords import cartesian_to_polar, polar_to_cartesian
from eclipse_v7.merge import NO_DATA
from eclipse_v7.utils import load_grayscale, apply_transform_single, compute_weighted_average, moon_median
from eclipse_v7.warp import (
    build_chains,
    grid_to_normalized_grid as _grid_to_normalized_grid,
    ref_to_source_grid as _ref_to_source_grid,
    warp_to_ref as _warp_to_ref,
)
...
UNSHARP_GAUSSIAN_SIGMAS = (1.0, 2.0, 4.0)
UNSHARP_WEIGHTS = (12.0, 6.9282, 4.0)
...
class Stage3Context:
    ...
    refined_registration: bool = False                         # True when cross_reg came from v7-stage2r.pkl (redone on calibrated radiance); set by load_inputs
    ...
    exposure_weights_crop: Optional[np.ndarray] = None          # exposure_weights sliced to crop bounds; set by crop_and_save_composite
...
def _scale_to_ref(exposure_times_sorted, gamma_by_pair, k):
    """LEGACY. Running product of per-pair exponents — the chain this version replaces.

    Kept only so `warp_merge_to_composite` stays runnable behind `--merge legacy` for
    side-by-side comparison. It is unsound: a single factor per pair can only describe two
    frames if `f` is a pure power law, and the errors of each link multiply.
    """
    if k == 0:
        return 1.0
    scale = 1.0
...
def load_inputs(ctx: Stage3Context, use_refined_registration: bool = True) -> None:
    ...
    `use_refined_registration` swaps in `v7-stage2r.pkl` — the cross-exposure transforms
...
def warp_merge_to_composite(ctx: Stage3Context) -> None:
    """LEGACY merge (v2 behaviour): per-pair exponent chain + bell weight on encoded values.

    Superseded by `merge.merge_to_composite`, and kept only so the two can be compared
    without a checkout — see `--merge legacy`. Requires `build_per_exposure_averages` first.
    """
    device = ctx.device
    ...
    exposures_with_chain, chain_tuples_by_exp = build_chains(
        exposure_times_sorted, avg_images, ctx.cross_reg, t_ref
    )
...
def fill_no_data(composite, no_data=None, fill: float = 0.0):
    ...
    Returns the input unchanged when there is nothing to fill, so the legacy merge — whose
    composite has no sentinel — passes through untouched.
    """
    if no_data is None:
...
def crop_and_save_composite(ctx: Stage3Context) -> None:
    ...
    if ctx.exposure_weights is not None:
        ctx.exposure_weights_crop = ctx.exposure_weights[:, r_lo : r_hi + 1, c_lo : c_hi + 1].copy()
        ctx.exposure_weights = None
    ...
    np.save(out_dir / "v7-stage3_composite.npy", radiance_crop)
    saved = [out_dir / "v7-stage3_composite.npy"]
    if ctx.variance_crop is not None:
        np.save(out_dir / "v7-stage3_variance.npy", ctx.variance_crop)
        saved.append(out_dir / "v7-stage3_variance.npy")
    if ctx.exposure_weights_crop is not None:
        # One full-resolution weight map per exposure — large by design (see the "full
        # per-exposure weight stack" discussion): exposure_weights_crop[k] is the weight
        # exposure_weight_times[k] contributed to composite_variance at every pixel, before
        # summing across exposures.
        np.save(out_dir / "v7-stage3_weights.npy", ctx.exposure_weights_crop)
        np.save(out_dir / "v7-stage3_weights_exposures.npy", ctx.exposure_weight_times)
        saved.append(out_dir / "v7-stage3_weights.npy")
        saved.append(out_dir / "v7-stage3_weights_exposures.npy")
    ...
    Image.fromarray((preview * 255).clip(0, 255).astype(np.uint8)).save(
        out_dir / "v7-stage3_composite_preview.png"
    )
    print("Saved " + ", ".join(str(p) for p in saved)
          + f", {out_dir / 'v7-stage3_composite_preview.png'}")
...
def radial_normalize_display(ctx: Stage3Context) -> None:
    ...
    # physical brightness already written to v7-stage3_composite.npy.
    ...
    Image.fromarray((ctx.display * 255).clip(0, 255).astype(np.uint8)).save(
        ctx.workdir / "v7-stage3_radial_normalize.png"
    )
    print(f"Saved {ctx.workdir / 'v7-stage3_radial_normalize.png'}")
...
def fft_unsharp_and_save(ctx: Stage3Context) -> None:
    ...
    out_png = ctx.workdir / "v7-stage3_radial_normalize_sharpen_fft_smoothed_diff.png"
...
def rgb_vignette_and_radial_pickle(ctx: Stage3Context) -> None:
    """RGB + vignette PNG and v7-stage3_radial.pkl sidecar."""
    ...
    out_png = ctx.workdir / "v7-stage3_rgb_rescaled.png"
...
def load_calibration(ctx: Stage3Context, source=None, path: Path = None):
    ...
    from eclipse_v7 import calib as CA

    path = (ctx.workdir / "v7-calib.pkl") if path is None else Path(path)
...
def run(workdir: Path, source=None, use_legacy_merge: bool = False) -> None:
    """Run full stage 3 pipeline; writes v7-stage3_* artifacts under workdir.

    Needs `source` for the radiometric merge (it reads frames through `load_radiance`) and a
    `v7-calib.pkl` alongside the stage pickles. `use_legacy_merge=True` reproduces v2.
    """
    from eclipse_v7.merge import merge_to_composite
