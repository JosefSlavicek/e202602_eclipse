...
import eclipse_v7.stage0  # noqa: F401 — pickle loads ImageInfo from this module

from PIL import Image

from eclipse_v7.utils import load_grayscale, apply_transform_single, compute_weighted_average, compose_transforms
...
def optimize_poses_and_debug(
    ...
    for exposure_time in sorted(exposure_groups.keys()):
        ...
        Image.fromarray(crop_single).save(debug_img_dir / f"v7-stage1_debugimg_{exposure_time:.6f}_random.png")
        Image.fromarray(crop_avg).save(debug_img_dir / f"v7-stage1_debugimg_{exposure_time:.6f}_average.png")
        frames = []
        for w in warped_list:
            c = (w[i_lo:i_hi, j_lo:j_hi].cpu().numpy() * 255).clip(0, 255).astype(np.uint8)
            frames.append(Image.fromarray(c))
        frames[0].save(
            debug_img_dir / f"v7-stage1_debugimg_{exposure_time:.6f}_anim.gif",
            save_all=True,
