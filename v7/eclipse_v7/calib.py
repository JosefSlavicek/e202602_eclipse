...
`v7/test_calib_synthetic.py` exercise the numerics in ~10 s on CPU.
"""
...
from eclipse_v7.utils import compute_weighted_average, moon_median
from eclipse_v7.warp import build_chains, sample_at_ref_points, warp_to_ref
...
def response_slope_lut(result: CalibResult, n: int = DENSE_LUT_POINTS):
    """(v grid, df/dv), floored so it can be divided by. Converts a stored-value error to light."""
    v, f = response_lut(result, n)
    slope = np.gradient(f, v)
    return v, np.maximum(slope, 1e-12)
