...
_STAGE1_DEBUG_ANIM_STRICT = re.compile(r"^v7-stage1_debugimg_(.+)_anim\.gif$")
_STAGE2_PAIR_STRICT = re.compile(
    r'^v7-stage2_pair_(\d+\.\d+)_(\d+\.\d+)_gamma(\d+\.\d+)\.gif$'
)
...
def symlink_stage1_debug_anim_gifs(workdir: Path, link_dir: Path | None = None) -> list[Path]:
    """
    For each strict-match `v7-stage1_debugimg_*_anim.gif` under `workdir`, create a symlink in
    ...
    created: list[Path] = []
    for src in sorted(workdir.glob("v7-stage1_debugimg*_anim.gif"), key=lambda p: p.name):
...
def display_clickable_stage1_debug_img_grid(columns: int = 8, width: int = 128, gif_dir: Path | None = None) -> None:
    """
    Show strict-match `v7-stage1_debugimg_*_anim.gif` under `gif_dir` (default: cwd) in a table
    ...
    for p in sorted(gif_dir.glob("v7-stage1_debugimg*_anim.gif"), key=lambda q: q.name):
        m = _STAGE1_DEBUG_ANIM_STRICT.match(p.name)
        if not m:
            continue
        chunk.append((p, m.group(1)))
    if not chunk:
        display(HTML("<p><em>No strict-match v7-stage1_debugimg_*_anim.gif files found.</em></p>"))
        return
    rows_html: list[str] = []
...
def symlink_stage2_pair_gifs(workdir: Path, link_dir: Path | None = None) -> list[Path]:
    ...
    link_dir.mkdir(parents=True, exist_ok=True)
    created: list[Path] = []
    for src in sorted(workdir.glob("v7-stage2_pair*.gif"), key=lambda p: p.name):
...
def display_clickable_stage2_pair_gif_grid(
    ...
    gif_dir = Path.cwd() if gif_dir is None else Path(gif_dir)
    chunk: list[tuple[Path, str]] = []
    for p in sorted(gif_dir.glob("v7-stage2_pair*.gif"), key=lambda q: q.name):
    ...
    if not chunk:
        display(HTML("<p><em>No strict-match v7-stage2_pair_*.gif files found.</em></p>"))
        return
    rows_html: list[str] = []
    for i in range(0, len(chunk), columns):
        row_cells: list[str] = []
