# v8 — bracketed exposures on one physical brightness scale

v8 is a self-contained copy of v7 (no cross-version imports; the `vN/eclipse_vN/`
convention v3 established), pared down to exactly one configuration: `pipeline.py` now takes
only `--nef-dir`, `--workdir`, `--exposure-group-subsample` and `--start-stage`; every other
former flag (merge mode, dark/flat model on/off, registration refinement, RNG seed, ...) is
hardcoded at what was v7's default, and the code paths those flags used to switch away from
(v2's legacy per-pair-exponent merge chain, the standalone test/debug/sweep scripts, the
notebook, the Jupyter display helpers) are deleted outright rather than left unreachable. The
produced composite is unchanged from v7's default run; only the surface is narrower. Runs are
also now seeded (see `eclipse_v8/device.py`'s `seed_everything`) — reruns on the same
machine/GPU/driver are bitwise reproducible.

v7 itself started as a self-contained copy of v6 and diverged in two ways, both still true
here:

- **NEF-only.** The JPG/atmosphere/inject input modes are gone; there is one source class
  (`NefSource`) and one input contract.
...
v8's calibration/registration machinery (unchanged) even though the input layer and merge
...
enters at all) and the calibration is refit on the result — one alternation step, always on.
Measured on the real JPEG bracket: 12 of 14 pairs move, median 0.36 px, max 0.72 px — 2 to 4
of the 0.18 px cells the grid search can resolve.

## Layout

| file | what it does |
|---|---|
| `eclipse_v8/calib.py` | recovers `ln f` at 64 knots + per-exposure shutter corrections (Debevec–Malik, sparse `lsqr`, corrections by alternation) |
| `eclipse_v8/rawprep.py` | measures per-frame overburn mask + merge weight once, straight off the unmodified raw decode, before dark/flat correction; also bakes dark/flat correction into a cached "corrected" array once fit, so the NEF is decoded exactly once |
| `eclipse_v8/merge.py` | per-frame brightness, inverse-variance merge, common moon blanking, `NO_DATA` sentinel; weight comes from `rawprep`'s per-frame maps, warped alongside the data (not derived from calibrated radiance); also keeps every exposure's own weight map (before summing) and writes it as `v8-stage3_weights.npy` — large by design, see below |
| `eclipse_v8/reregister.py` | redoes the cross-exposure registration on calibrated radiance; writes `v8-stage2r.pkl` |
| `eclipse_v8/warp.py` | reference-grid warping and the cross-exposure chain, shared by the calibration and the merge |
| `eclipse_v8/inputs.py` | `NefSource`: finds `lights/`/`darks/`/`flats/` under the folder it's given; `load_gray` reads the rawprep-corrected cache (`set_cache_dirs`), `load_overburn`/`load_weight` read the raw-measured cache, `load_radiance` divides by the effective exposure (shutter-corrected via `set_calibration`) |
| `eclipse_v8/darkcal.py` | per-pixel dark-current model `bias + rate * t`, fit by OLS over every `darks/` frame at once against the plain EXIF-reported `t` — deliberately no shutter-time correction (see its module docstring for why matched-by-label subtraction doesn't need one); writes `v8-dark.pkl` |
| `eclipse_v8/flatcal.py` | per-pixel flat field, **high-frequency only and brightness-dependent**: `flats/` is grouped by exposure time (dropping frames with too many saturated pixels and whole groups with too few survivors), each group gets the usual high-pass treatment (sigma=24px Gaussian blur, mirror-padded, subtracted from each dark-subtracted frame first), then every pixel gets its own least-squares line `a(x,y) + b(x,y)*value` fit across groups (falling back to a single brightness-independent map if fewer than 2 groups qualify); `FlatModel.evaluate(value)` clamps the result to `[0.95, 1.05]` at apply time; writes `v8-flat.pkl` |
| `eclipse_v8/stage3.py` | display chain, unchanged, plus the merge→display handoff and `DISPLAY_GAMMA` |
...
python v8/pipeline.py                                    # full run
python v8/pipeline.py --nef-dir /path/to/raws            # lights/darks/flats live under this dir
python v8/pipeline.py --workdir /path/to/run              # default /home/slavik/tmp/eclipse_v8_run
python v8/pipeline.py --exposure-group-subsample 2        # ~half the exposure groups, faster iteration
python v8/pipeline.py --start-stage 3                     # resume from stage 0-2's pickles
```

Everything else (merge mode, dark/flat model on/off, registration refinement, RNG seed) is
fixed in code at what was v7's default — see the top of this README.

## Per-exposure weight stack

`v8-stage3_weights.npy` — shape `(n_exp, H_crop, W_crop)` float32, one full-resolution
weight map per exposure in the merge's cross-exposure chain, cropped identically to
`v8-stage3_composite.npy`. `v8-stage3_weights.npy[k]` is exactly the weight exposure
`v8-stage3_weights_exposures.npy[k]` (that array's reported shutter time, seconds)
contributed to `composite_variance` at every pixel, *before* summing across exposures —
`v8-stage3_weights.npy.sum(axis=0) == 1 / v8-stage3_variance.npy` at every covered pixel.
...
`v8-stage3_weights_exposures.npy` gives the dominant exposure per pixel cheaply, if that is
all you need.
