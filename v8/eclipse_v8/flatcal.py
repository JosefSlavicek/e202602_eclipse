...
this exposure (`_group_weights_t`) -- zero wherever a pixel had zero valid frames at all, and also
...
itself (`_masked_blur_t`), which excludes zero-valid-frame pixels from its convolution outright
...
run as `torch` tensor ops on GPU when one is visible (`_default_device`), CPU otherwise.
`FlatModel` itself is still plain numpy -- picklable, no torch -- only the fit's internals
moved to GPU.
...
def _masked_blur_t(y: torch.Tensor, valid: torch.Tensor, sigma: float) -> torch.Tensor:
    ...
    pulling it toward the clipped value. Mirror-pads at the border for the same reason: without
    it, blur near an edge would mix in fabricated zeros and bias the result there. Where a pixel
    has no valid neighbors at all within the kernel (only possible with a contiguous overburn
    region far larger than `sigma`), falls back to the unmasked blur there rather than dividing
    by ~0. Operates in place on the GPU/CPU device `y` already lives on -- what the streamed fit
    loop calls directly, so per-frame data never round-trips through numpy. The blur itself runs
    in `_BLUR_DTYPE` (fp32); the result is cast back to `y`'s own dtype (fp64 in the streamed
    fit)."""
    orig_dtype = y.dtype
...
def _group_weights_t(
    ...
    the blur being computed from a neighborhood already biased by a nearby cluster of
    mostly-invalid pixels, even at a pixel that was fine in every one of its own frames. The
    blur runs in `_BLUR_DTYPE` (fp32); the returned weight is float64, matching the fit's
    sums."""
...
def _group_average_t(
    ...
    mean -- the exact averaging `fit_flat_model_from_stack` uses per exposure group. `raw_frames`
    is an iterable of (H, W) arrays. `dark_bias_t`/`dark_rate_t` are (H, W) arrays evaluated at
    `t`, or None. Returns `(avg_x, valid_group, bad_frac, n_overburn_total)`, tensors on `device`
    plus a plain int, so the fit loop that calls this directly never round-trips through numpy:
    `avg_x` is dark-subtracted (if given) and 0 wherever `valid_group` is False (no frame was
    ever valid there); `bad_frac` is the fraction of this group's frames invalid at each pixel
    (see `_group_weights_t`)."""
    sum_w = sum_wx = None
    n = 0
    n_overburn_total = 0
