...
import eclipse_v8.stage0  # noqa: F401
from eclipse_v8.stage0 import find_moon
from eclipse_v8.coords import cartesian_to_polar, polar_to_cartesian
from eclipse_v8.merge import NO_DATA
from eclipse_v8.utils import moon_median
...
class Stage3Context:
    ...
    gamma_by_pair: Any = None                                  # (t0,t1) -> gamma brightness scaling from stage2; diagnostic only (see load_inputs); loaded by load_inputs
    refined_registration: bool = False                         # True when cross_reg came from v8-stage2r.pkl (redone on calibrated radiance); set by load_inputs
...
def load_inputs(ctx: Stage3Context, use_refined_registration: bool = True) -> None:
    ...
    `use_refined_registration` swaps in `v8-stage2r.pkl` — the cross-exposure transforms
    redone on calibrated radiance — when the pipeline produced one. `gamma_by_pair` always
    comes from stage 2; it is loaded onto `ctx` for the diagnostic print below only.
    """
    with open(ctx.workdir / "v8-stage1.pkl", "rb") as fd:
        ctx.exposure_groups = pickle.load(fd)
        ctx.reg = pickle.load(fd)
        ctx.opt_results = pickle.load(fd)
    with open(ctx.workdir / "v8-stage2.pkl", "rb") as fd:
        ctx.cross_reg = pickle.load(fd)
        ctx.gamma_by_pair = pickle.load(fd)
    refined_pkl = ctx.workdir / "v8-stage2r.pkl"
...
def crop_and_save_composite(ctx: Stage3Context) -> None:
    ...
    np.save(out_dir / "v8-stage3_composite.npy", radiance_crop)
    saved = [out_dir / "v8-stage3_composite.npy"]
    if ctx.variance_crop is not None:
        np.save(out_dir / "v8-stage3_variance.npy", ctx.variance_crop)
        saved.append(out_dir / "v8-stage3_variance.npy")
    if ctx.exposure_weights_crop is not None:
        ...
        np.save(out_dir / "v8-stage3_weights.npy", ctx.exposure_weights_crop)
        np.save(out_dir / "v8-stage3_weights_exposures.npy", ctx.exposure_weight_times)
    ...
    Image.fromarray((preview * 255).clip(0, 255).astype(np.uint8)).save(
        out_dir / "v8-stage3_composite_preview.png"
    )
    print("Saved " + ", ".join(str(p) for p in saved)
          + f", {out_dir / 'v8-stage3_composite_preview.png'}")
...
def radial_normalize_display(ctx: Stage3Context) -> None:
    ...
    # physical brightness already written to v8-stage3_composite.npy.
    ...
    Image.fromarray((ctx.display * 255).clip(0, 255).astype(np.uint8)).save(
        ctx.workdir / "v8-stage3_radial_normalize.png"
    )
    print(f"Saved {ctx.workdir / 'v8-stage3_radial_normalize.png'}")
...
def fft_unsharp_and_save(ctx: Stage3Context) -> None:
    ...
    out_png = ctx.workdir / "v8-stage3_radial_normalize_sharpen_fft_smoothed_diff.png"
...
def rgb_vignette_and_radial_pickle(ctx: Stage3Context) -> None:
    """RGB + vignette PNG and v8-stage3_radial.pkl sidecar."""
    ...
    out_png = ctx.workdir / "v8-stage3_rgb_rescaled.png"
...
def load_calibration(ctx: Stage3Context, source=None, path: Path = None):
    ...
    path = (ctx.workdir / "v8-calib.pkl") if path is None else Path(path)
