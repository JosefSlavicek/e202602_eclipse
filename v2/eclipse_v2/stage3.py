...
import eclipse_v2.stage0  # noqa: F401
from eclipse_v2.stage0 import find_moon
from eclipse_v2.coords import cartesian_to_polar, polar_to_cartesian
from eclipse_v2.utils import load_grayscale, apply_transform_single, compute_weighted_average, moon_median
...
def load_inputs(ctx: Stage3Context) -> None:
    ...
    with open(ctx.workdir / "v2-stage1.pkl", "rb") as fd:
        ctx.exposure_groups = pickle.load(fd)
        ctx.reg = pickle.load(fd)
        ctx.opt_results = pickle.load(fd)
    with open(ctx.workdir / "v2-stage2.pkl", "rb") as fd:
...
def crop_and_save_composite(ctx: Stage3Context) -> None:
    ...
    np.save(out_dir / "v2-stage3_composite.npy", composite_crop)
    v_min = np.percentile(
    ...
    Image.fromarray((preview * 255).clip(0, 255).astype(np.uint8)).save(
        out_dir / "v2-stage3_composite_preview.png"
    )
    print(f"Saved {out_dir / 'v2-stage3_composite.npy'} (float32), {out_dir / 'v2-stage3_composite_preview.png'}")
...
def radial_normalize_display(ctx: Stage3Context) -> None:
    ...
    Image.fromarray((ctx.display * 255).clip(0, 255).astype(np.uint8)).save(
        ctx.workdir / "v2-stage3_radial_normalize.png"
    )
    print(f"Saved {ctx.workdir / 'v2-stage3_radial_normalize.png'}")
...
def fft_unsharp_and_save(ctx: Stage3Context) -> None:
    ...
    out_png = ctx.workdir / "v2-stage3_radial_normalize_sharpen_fft_smoothed_diff.png"
...
def rgb_vignette_and_radial_pickle(ctx: Stage3Context) -> None:
    """RGB + vignette PNG and v2-stage3_radial.pkl sidecar."""
    ...
    out_png = ctx.workdir / "v2-stage3_rgb_rescaled.png"
    Image.fromarray((display_rgb * 255).round().clip(0, 255).astype(np.uint8)).save(out_png)
    print(f"Saved {out_png}")


def run(workdir: Path) -> None:
    """Run full stage 3 pipeline; writes v2-stage3_* artifacts under workdir."""
