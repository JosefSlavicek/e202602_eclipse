...
# Strict: v0-eda02_debugimg_<exposure>_anim.gif (exposure substring used in captions as-is)
_EDA02_DEBUG_ANIM_STRICT = re.compile(r"^v0-eda02_debugimg_(.+)_anim\.gif$")
...
def symlink_eda02_debug_anim_gifs(workdir: Path, link_dir: Path | None = None) -> list[Path]:
    """
    For each strict-match `v0-eda02_debugimg_*_anim.gif` under `workdir`, create a symlink in
    ...
    created: list[Path] = []
    for src in sorted(workdir.glob("v0-eda02_debugimg*_anim.gif"), key=lambda p: p.name):
...
def display_clickable_eda02_debug_img_grid(columns: int = 8, width: int = 128, gif_dir: Path | None = None) -> None:
    """
    Show strict-match `v0-eda02_debugimg_*_anim.gif` under `gif_dir` (default: cwd) in a table
    ...
    for p in sorted(gif_dir.glob("v0-eda02_debugimg*_anim.gif"), key=lambda q: q.name):
        m = _EDA02_DEBUG_ANIM_STRICT.match(p.name)
        if not m:
            continue
        chunk.append((p, m.group(1)))
    if not chunk:
        display(HTML("<p><em>No strict-match v0-eda02_debugimg_*_anim.gif files found.</em></p>"))
        return
