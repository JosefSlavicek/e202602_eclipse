"""Stage 3: per-exposure averages + cross-exposure registration and gamma (eda03)."""
...
def stage3_load(eda02_pkl: Path):
...
def stage3_moon_median_table(exposure_groups: dict) -> dict:
    moon_by_exp = {}
...
def stage3_cross_exposure_consecutive_pairs(
    exposure_times_sorted: list,
    avg_images: dict,
    moon_by_exp: dict,
    device: torch.device,
...
def stage3_save_pickle(out_pkl: Path, pairs_results: list) -> Path:
...
def run_stage3(eda02_pkl: Path, out_pkl: Path, pair_gif_dir: Path) -> Path:
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for stage 3.")
    device = torch.device("cuda")

    exposure_groups, _reg, opt_results = stage3_load(eda02_pkl)
    moon_by_exp, exposure_times_sorted = stage3_moon_median_table(exposure_groups)
    avg_images = stage3_fullsize_averages(exposure_groups, exposure_times_sorted, opt_results, device)
    pairs_results = stage3_cross_exposure_consecutive_pairs(
        exposure_times_sorted, avg_images, moon_by_exp, device, pair_gif_dir
    )
    return stage3_save_pickle(out_pkl, pairs_results)
