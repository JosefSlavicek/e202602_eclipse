"""Stage 5: composite merge, radial processing, sharpen, RGB (eda05)."""
...
STAGE5_PATCH_SIDE = 256
STAGE5_PATCH_STRIDE = 8
STAGE5_FFT_INWARD_MEDIAN_SPAN = 32
STAGE5_UNSHARP_GAUSSIAN_SIGMAS = (2.0, 4.0, 8.0)
STAGE5_UNSHARP_WEIGHTS = (8.0, 8.0, 0.5)


def stage5_count_fft_patch_placements(height: int, width: int) -> tuple[int, int, int]:
    """How many patch origins (row starts × col starts) the FFT sharpen scans; total per blur sigma."""
    a, stride = STAGE5_PATCH_SIDE, STAGE5_PATCH_STRIDE
...
def run_stage5(workdir: Path) -> None:
    """Run full eda05 pipeline; writes v0-eda05_* artifacts under workdir."""
    PKL_DIR = workdir
    import pickle as _pickle
    with open(workdir / "v0-eda02.pkl", "rb") as fd:
        exposure_groups = _pickle.load(fd)
        reg = _pickle.load(fd)
        opt_results = _pickle.load(fd)
    with open(workdir / "v0-eda03.pkl", "rb") as fd:
        cross_reg = _pickle.load(fd)
        gamma_by_pair = _pickle.load(fd)
    device = torch.device("cuda")

    exposure_times_sorted = sorted(exposure_groups.keys())
    exposure_times_sorted = exposure_times_sorted[2:]
    ...
    np.save(out_dir / "v0-eda05_composite.npy", composite_crop)
    ...
    Image.fromarray((preview * 255).clip(0, 255).astype(np.uint8)).save(out_dir / "v0-eda05_composite_preview.png")
    ...
    if False:
        ...
        for i in range(10):
            ...
            Image.fromarray((preview * 255).clip(0, 255).astype(np.uint8)).save(out_dir / f"v0-eda05_composite_preview_{i}.png")
            lv_max /= 2
        raise
    ...
    def image_to_polar_f64(img, center, radius_min, radius_max, n_r, n_theta, device, dtype):
        ...
        mask = (i_src >= 2) & (i_src < H-2) & (j_src >= 2) & (j_src < W-2)
    ...
    # Extrapolate partial polar rows from a mean angular template (GPU vectorized), then sliding-window mean.
    # i=0 is outer radius; first fully valid row is smallest i with all-θ valid; all rows below must be fully valid.
    ...
    # Still require all 100 rows to be well-conditioned (annulus sanity).
    ...
    for i in tqdm.tqdm(range(i_first)): # this can be vectorized, but i was debuging it and let the loop here
        if n_per[i] < 64:
             continue
    ...
    plt.tight_layout()
    plt.close(fig)
    Image.fromarray((display * 255).clip(0, 255).astype(np.uint8)).save(Path(PKL_DIR) / "v0-eda05_radial_normalize.png")
    print(f"Saved {Path(PKL_DIR) / 'v0-eda05_radial_normalize.png'}")
    # eda05: 3-way FFT-smoothed unsharp — Gaussian blur σ = 2, 4, 8; one FFT-smoothed residual per σ
    
    inward_median_span = STAGE5_FFT_INWARD_MEDIAN_SPAN
    strength_r2, strength_r4, strength_r8 = STAGE5_UNSHARP_WEIGHTS
    ...
    def prepare_display_for_blur_moon_polar(
        ...
        w = torch.ones_like(polar_filled[:, :1])
        w2 = torch.arange(n_r, dtype=torch.float32, device=w.device).reshape(-1, 1)
        w2 = torch.exp(-((w2 - i_border) / 8)**2)
        w[i_border:, :] = w2[i_border:, :]
        polar_filled *= w
    ...
    def fft_bw_energy_protect_mask(Fs_bw, energy_frac=0.9):
        """fftshift grid: greedy bins by descending |F|² until cum. energy >= energy_frac * total; DC always included."""
        a = Fs_bw.shape[0]
    ...
    def _radial_blend_fancy_plain(fancy_np, plain_np, mi, mj, moon_r, blur_sigma):
        """Weight fancy near the limb; plain isotropic blur farther out (linear ramp over one sigma)."""
        H, W = plain_np.shape
    ...
    def _periodic_gaussian_kernel_1d(n, sigma, device, dtype):
        """Unit-sum nonnegative kernel on Z/nZ (distance min(k, n-k))."""
        n = int(n)
    ...
    def _fancy_polar_blur_cartesian(display_np, blur_sigma, mi_crop, mj_crop, moon_r, device, dtype=torch.float32):
        """Polar band blur on GPU (cyclic θ via FFT; r-blur via matmul with full σ in row index, excluding r<moon_r sources and renormalizing), then warp to Cartesian."""
        H, W = display_np.shape
        center = (float(mi_crop), float(mj_crop))
        radius_min = 0.0
        radius_max = _blur_dist_to_corners(mi_crop, mj_crop, H, W)
        ...
        polar, _ = _blur_image_to_polar(img, center, radius_min, radius_max, n_r, n_theta, device, dtype)
        polar_h = polar.clone()
        ...
        cart = _blur_polar_to_cartesian(polar_v, center, radius_min, radius_max, H, W, device, dtype)
        return cart.detach().cpu().numpy()
    
    
    def sliding_diff_smooth_for_sigma(
        display,
        display_for_blur,
        blur_sigma,
        inward_median_span,
        ...
        for r0 in tqdm.tqdm(row_starts, desc=f"FFT-smooth diff sigma={blur_sigma}"):
            for c0 in col_starts:
                ...
                if f0 == 0 or f1 == 0 or min(f0, f1) < area_tol:
                    ...
                    protect = fft_bw_energy_protect_mask(Fs_bw, protect_energy_frac)
                    F_fft = torch.fft.fft2(p_w)
                    Fs = torch.fft.fftshift(F_fft)
                    Fs_save = Fs.clone()
                    Fs_work = Fs.clone()
                    Fs_work[protect] = 0
                    Fs_proc = fftshift_complex_polar_preprocess(Fs_work, inward_median_span)
                    G = make_fft_mask2_percentile_for_tile(Fs_proc, tc_i, tc_j, mi_crop, mj_crop, moon_r)
                    Fs_out = Fs_proc * G
    ...
    a = STAGE5_PATCH_SIDE
    stride = STAGE5_PATCH_STRIDE
    dev = device
    
    #display_for_blur = prepare_display_for_blur_moon_polar(display, mi_crop, mj_crop, moon_r)
    display_for_blur = display #prepare_display_for_blur_moon_polar2(display, mi_crop, mj_crop, moon_r, a=a)
    ...
    out_png = Path(PKL_DIR) / "v0-eda05_radial_normalize_sharpen_fft_smoothed_diff.png"
    ...
    R = gray
    G = 0.16 + 0.84 * gray  # (0, 1) -> (0.18, 1)
    B = 0.30 + 0.70 * gray  # (0, 1) -> (0.28, 1)
    display_rgb = np.clip(np.stack([R, G, B], axis=-1), 0.0, 1.0)
    
    # Moon-centered vignette: g(r)=exp(-0.5*(r/sigma)^2), sigma = dist to farthest crop corner;
    # scale=1 at moon center, dim_q at corners (0 < dim_q < 1).
    dim_q = (0.25, 0.28, 0.37)
    H, W = display_rgb.shape[0], display_rgb.shape[1]
    ci, cj = float(mi_crop), float(mj_crop)
    sigma = max(
        np.hypot(ci - 0, cj - 0),
    ...
    out_png = Path(PKL_DIR) / "v0-eda05_rgb_rescaled.png"
    ...
    out_path = Path(PKL_DIR) / "v0-eda05_radial.pkl"
