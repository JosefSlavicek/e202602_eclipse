...
@dataclass
class Stage5Context:
    """Mutable state passed through stepped stage-5 functions (notebooks) or `run_stage5` (one-shot)."""
...
def _apply_transform_single(img, shift_i, shift_j, angle_deg, device):
    H, W = img.shape
    ci, cj = H / 2.0, W / 2.0
...
def _compute_weighted_average(group, abs_xy, abs_angle_t, device, epsilon=1e-6):
    n = len(group)
    sum_img = None
    sum_mask = None
    for j in range(n):
        img_j = _load_grayscale(group[j], device)
        ...
        w_img = _apply_transform_single(img_j, x_j, y_j, theta_j_deg, device)
        w_mask = _apply_transform_single(mask_j, x_j, y_j, theta_j_deg, device)
        if sum_img is None:
...
def _image_to_polar_f64(img, center, radius_min, radius_max, n_r, n_theta, device, dtype):
    H, W = img.shape
...
def _polar_to_cartesian_f64(polar, center, radius_min, radius_max, H, W, device, dtype):
...
def stage5_load_inputs(ctx: Stage5Context) -> None:
    ...
    ctx.moon_ref = _moon_median(ctx.exposure_groups[ctx.t_ref])
...
def stage5_build_per_exposure_averages(ctx: Stage5Context) -> None:
    """GPU stack mean per exposure (moon blackened, warped with stage-2 poses)."""
    device = ctx.device
    ...
    for exp in tqdm.tqdm(ctx.exposure_times_sorted, desc="Averaged images"):
        ...
        ctx.avg_images[exp], ctx.avg_masks[exp] = _compute_weighted_average(group, abs_xy, abs_angle_t, device)
...
def stage5_warp_merge_to_composite(ctx: Stage5Context) -> None:
...
def stage5_crop_and_save_composite(ctx: Stage5Context) -> None:
    ...
    Image.fromarray((preview * 255).clip(0, 255).astype(np.uint8)).save(
        out_dir / "v0-eda05_composite_preview.png"
    )
...
def stage5_radial_normalize_display(ctx: Stage5Context) -> None:
    ...
    polar_img, valid = _image_to_polar_f64(img, center, radius_min, radius_max, n_r, n_theta, device, dtype)
    ...
    polar_display, _ = _image_to_polar_f64(display_t, center, radius_min, radius_max, n_r, n_theta, device, dtype)
    ...
    Image.fromarray((ctx.display * 255).clip(0, 255).astype(np.uint8)).save(
        ctx.workdir / "v0-eda05_radial_normalize.png"
    )
    print(f"Saved {ctx.workdir / 'v0-eda05_radial_normalize.png'}")


def stage5_fft_unsharp_and_save(ctx: Stage5Context) -> None:
    ...
    out_png = ctx.workdir / "v0-eda05_radial_normalize_sharpen_fft_smoothed_diff.png"
...
def stage5_rgb_vignette_and_radial_pickle(ctx: Stage5Context) -> None:
