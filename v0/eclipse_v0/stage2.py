"""Stage 2: brightness + triplet pruning, per-exposure pose optimization (eda02)."""
...
def prune_brightness_outliers(exposure_groups, reg, std_reduction_factor=3.0):
    for exp_key in sorted(exposure_groups.keys()):
        ...
        while True:
            ...
            print(
                f"prune_brightness: exp={exp_key:.5f} removed image (idx {k}) {removed_ii.path.name} "
                f"brightness={removed_ii.avg_brightness:.6f} group_std {std_b:.6f} -> {new_std:.6f}"
            )


def prune_failed(exposure_groups, reg, threshold):
    ...
    for exp_key in exposure_times:
        ...
        while True:
            ...
            for (i, j), v in reg_exp_old.items():
                ...
                j_new = j if j < k else j - 1
                reg[(exp_key, i_new, j_new)] = v
            print(f"prune_failed: exp={exp_key:.5f} removed image (idx {k}) {removed_ii.path.name}")
...
def stage2_load(eda00_pkl: Path):
...
def stage2_prune_groups(exposure_groups, reg) -> None:
    ...
    prune_failed(exposure_groups, reg, 1.0)


def stage2_optimize_poses_and_debug(
    exposure_groups,
    reg,
    device: torch.device,
    debug_img_dir: Path,
...
def stage2_save_pickle(out_pkl: Path, exposure_groups, reg: dict, opt_results: dict) -> Path:
...
def run_stage2(eda00_pkl: Path, out_pkl: Path, debug_img_dir: Path) -> Path:
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for stage 2.")
    device = torch.device("cuda")

    exposure_groups, reg = stage2_load(eda00_pkl)
    stage2_prune_groups(exposure_groups, reg)
    opt_results = stage2_optimize_poses_and_debug(exposure_groups, reg, device, debug_img_dir)
    return stage2_save_pickle(out_pkl, exposure_groups, reg, opt_results)
